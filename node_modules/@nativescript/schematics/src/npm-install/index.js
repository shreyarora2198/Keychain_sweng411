...
const validateInput = (options) => () => {
    if ((options.dependencies || options.devDependencies) && options.json) {
        throw new schematics_1.SchematicsException('Too many parameters provided to [npm-install] schematic. Use either json, or dependencies with devDependencies, but not all 3 of them');
    }
};
const updatePackageJson = (options) => (tree) => {
    if (!options.dependencies && !options.devDependencies && !options.json)
        return;
    const packageJson = utils_1.getPackageJson(tree, options.workingDirectory);
    ...
    utils_1.overwritePackageJson(tree, packageJson, options.workingDirectory);
};
...
function parseStringToDependencies(param) {
    if (!param || param === '') {
        return [];
    }
    const list = param.replace(' ', '').split(',');
    const result = list.map(item => {
        const index = item.lastIndexOf('@');
        if (index === -1) {
            throw new schematics_1.SchematicsException(`invalid value in dependencies ${item}. Expected something like name@version`);
        }
        return {
...
function parseObjectToDependencies(dependencies) {
    if (!dependencies) {
        return [];
    }
    const result = [];
...
function addNpmPackages(dependencies, npmPackages) {
...
function checkForDuplicatePackages(packageJson, newDependencies) {
...
const installNpmModules = (workingDirectory) => (_tree, context) => {
    context.addTask(new tasks_1.NodePackageInstallTask(workingDirectory));
};
