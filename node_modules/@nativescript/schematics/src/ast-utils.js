...
function addSymbolToComponentMetadata(source, componentPath, metadataField, symbolName) {
    return addSymbolToDecoratorMetadata(source, componentPath, metadataField, symbolName, 'Component', '@angular/core');
}
exports.addSymbolToComponentMetadata = addSymbolToComponentMetadata;
...
function getFunctionParams(source, name) {
    const node = findNode(source, [
        { kind: ts.SyntaxKind.CallExpression, name },
    ]);
    if (node.getChildCount() !== 4) {
        throw new schematics_1.SchematicsException(`Couldn't find function params for ${name} in ${node.getSourceFile().fileName}`);
    }
